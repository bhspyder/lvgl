...
void lv_display_refr_timer(lv_timer_t * tmr)
{
    ...
    if((lv_display_is_double_buffered(disp_refr) && disp_refr->render_mode == LV_DISPLAY_RENDER_MODE_DIRECT) || disp_refr->sync_cb) {
        uint32_t i;
...
static void refr_sync_areas(void)
{
    /*Do not sync if not direct and no sync callback set*/
    if(disp_refr->render_mode != LV_DISPLAY_RENDER_MODE_DIRECT && !disp_refr->sync_cb) return;

    /*Do not sync if not double buffered and no sync callback set*/
    if(!lv_display_is_double_buffered(disp_refr) && !disp_refr->sync_cb) return;

    /* Do not sync if no sync callback set */
    if (!disp_refr->sync_cb) return;
